...
export class AmbitComponent implements OnInit {
  ...
  save($ev, value: any) {
    console.log(this.formambit);

    $ev.preventDefault();
    // tslint:disable-next-line:forin
    for (const c in this.formambit.controls) {
      this.formambit.controls[c].markAsTouched();
    }
    if (this.formambit.valid) {
      if (this.formambit.value.id === '0') {
        this._ambitService.create(this.formambit.value).subscribe(resp => {
          this.setPage({offset: 0 });
        });
      } else {
        this._ambitService.update(this.formambit.value).subscribe(resp => {
          this.setPage({offset: 0 });
        });
      }
      this.formambit.reset();
      this.modalService.dismissAll();
    }
    console.log(this.formambit.value);
  }
  ...
  editbyid(content, id: string) {
    ...
    if (id) {
      this._ambitService.get(id).subscribe((resp: any) => {
        ...
        this.formambit.setValue(resp.data);
      });
    }
    this.modalService
      .open(content, {
