...
export class PersonComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public _positionService: EmployeePositionService,
    ...
    if(form.value.id ==="0") {
      this.personService.create(form.value)
        .subscribe((resp : any) => {
          ...
          if(resp.success){
            let employee = new Employee(resp.data.id,form.value.position,null);
  ...
  loadPositions(){
    this._positionService.list().subscribe((resp:any)=>{
