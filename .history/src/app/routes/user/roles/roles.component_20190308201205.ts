...
export class RolesComponent implements OnInit {
  ...
  save(form?: NgForm) {
    ...
    if (form.value.id != 0) {
      ...
      this._rolService.create(form.value)
        .subscribe((res: any) => {
          // console.log(res, 'role.component.ts (105)');
          
          // if (res.success === true) {
          //   this.toasterService.pop('success', 'AGREGAR ROL', 'ROL AGREGADO CON EXITO!');
          // } else {
          //   this.toasterService.pop('warning', 'AGREGAR ROL', 'Se produjo un error!');
          //   //console.log(res.error.message , 'respuesta erronea');
          // }
          this.setPage({ offset: 0 });
          this.resetForm(form);
        });
  ...
  CloseModal(data: string, form?: NgForm) {
    console.log('CLOSE MO', form.value);
  ...
  deleteRole(id: string) {
    this._rolService.delete(id).subscribe((resp: any) => {
      if (resp.success === true) {
        this.toasterService.pop('info', 'ELIMINAR', 'Rol eliminado con exito!');
      this.setPage({offset: 0 });
    }
    });
  }
