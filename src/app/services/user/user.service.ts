...
export class UserService {
    ...
    return this.http.post(url,user)
              .map( (resp: any) => {
                if(resp.success == true){
                  let tokenInfo = this.getDecodedAccessToken(resp.data.token); // decode token
                ...
                this.toasterService.pop('warning', 'Error de Accesos', err.error.message);
  ...
  delete(id:string){
    if(this.user.id === id) {
      this.toasterService.pop('success', 'Usuario Creado', 'No se puede eliminar el usuario del actual.' );
      // this.toastr.info('', 'ELIMINACIÓN DE USUARIO', { timeOut: 3000,positionClass: 'toast-top-right'});
